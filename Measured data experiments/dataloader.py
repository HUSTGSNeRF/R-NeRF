...
def rssi2amplitude(rssi):
    ...
    """
    return (rssi+67.2020)/(34.5790)


def amplitude2rssi(amplitude):
    """convert amplitude to rssi
    """
    return amplitude*(34.5790)-67.2020
        ...
        for idx in tqdm(self.dataset_index, total=len(self.dataset_index)):
            ...
            for i_gateway, rssi in enumerate(rssis):
                if rssi != -100:
                    ...
                    gateway_ray_o2 = gateways_ray_o2[i_gateway].view(-1)  # [3]

                    gateway_rays_d2 = gateways_rays_d2[i_gateway].view(-1)  # [n_rays x 3]

                    nn_inputs[data_counter] = torch.cat([tx_pos, gateway_ray_o1, gateway_rays_d1, ris_pos, gateway_ray_o2, gateway_rays_d2], dim=-1)
